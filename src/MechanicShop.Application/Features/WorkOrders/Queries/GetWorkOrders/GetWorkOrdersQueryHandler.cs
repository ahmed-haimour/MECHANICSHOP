    ...
    : IRequestHandler<GetWorkOrdersQuery, Result<PaginatedList<WorkOrderListItemDto>>>
{
    ...
    private static IQueryable<WorkOrder> ApplySearchTerm(IQueryable<WorkOrder> query, string searchTerm)
    {
        ...
        return query.Where(wo =>
            (wo.Vehicle != null && (
                (wo.Vehicle!.Make ?? string.Empty).ToLower().Contains(normalized) ||
                (wo.Vehicle!.Model ?? string.Empty).ToLower().Contains(normalized) ||
                (wo.Vehicle!.LicensePlate ?? string.Empty).ToLower().Contains(normalized)
            )) ||
            (wo.Labor != null && (
