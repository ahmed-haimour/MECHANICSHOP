    ...
    : IRequestHandler<AssignLaborCommand, Result<Updated>>
{
    ...
    public async Task<Result<Updated>> Handle(AssignLaborCommand request, CancellationToken ct)
    {
        var workOrder = await _context.WorkOrders
                .FirstOrDefaultAsync(a => a.Id == request.WorkOrderId, ct);
        ...
        if (await _workOrderValidator.IsLaborOccupied(request.LaborId, request.WorkOrderId, workOrder.StartAtUtc, workOrder.EndAtUtc))
        {
